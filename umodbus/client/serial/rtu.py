...
def send_over_tcp(adu, sock):
    import binascii
    # print(len(adu))
    k = binascii.b2a_hex(adu)
    sock.sendall(adu)
    # exception_adu_size = 3
    exception_adu_size = 1000
    from textwrap import wrap
    response_error_adu = recv_exactly(sock.recv, exception_adu_size)
    print(response_error_adu)
    stt = str(binascii.b2a_hex(response_error_adu))
    print('recived bytes -- >', stt.replace('', ''))
    # print("length of recived bytes", len(stt))

    data_list = wrap(stt[8:len(stt) - 4], 4)

    data_list = [s for s in data_list if s.isalnum()][:-1]
    # print(len(data_list), data_list)
    raw = [int(i, 16) for i in data_list]
