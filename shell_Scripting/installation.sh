...
file_down(){
  echo "Downloading Hash_codes "
  wget  -q "https://docs.google.com/spreadsheets/d/1ACz6hoBqhm0AI5IUbzsDw3IwGXCiwB2e-0gQF1BEKjo/export?format=csv&gid=808450103" -O Hash_codes.csv
  if [ -f /home/"$(echo "$(echo "$(w -h -s)" | cut -c1-9)" | xargs)"/PycharmProjects/CodingManiac/shell_Scripting/Hash_codes.csv ];then echo "Hash_codes downloaded";else echo "error downloading Hash_codes";exit 90;fi
}
Hashi(){
  INPUT=Hash_codes.csv
  OLDIFS=$IFS
  IFS=','
  if [ ! -f $INPUT ];then file_down ;fi
